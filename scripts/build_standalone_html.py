...
def build_standalone_html():
    print("Building mobile-first fully responsive standalone HTML...")
...
<head>
  ...
  <title>PET Clima Araraquara - Mapa Interativo de Impacto Climático</title>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; touch-action: manipulation; }}
    ...
    .brand-icon {{
      ...
      font-size: 18px; box-shadow: 0 0 12px rgba(16, 185, 129, 0.4);
    }}
    ...
    .btn-header:hover {{ background: rgba(255, 255, 255, 0.16); }}
    ...
    .kpi-value {{ font-family: var(--font-heading); font-size: 17px; font-weight: 700; color: var(--text-primary); }}
    ...
    .units-list-container::-webkit-scrollbar {{ width: 4px; }}
    .units-list-container::-webkit-scrollbar-thumb {{ background: rgba(255, 255, 255, 0.15); border-radius: 2px; }}
    ...
    .unit-rank {{ font-family: var(--font-heading); font-size: 11px; font-weight: 700; color: var(--accent-primary); }}
    .unit-title {{ font-size: 12px; font-weight: 600; color: var(--text-primary); line-height: 1.25; }}
    .unit-type {{ font-size: 10px; color: var(--text-secondary); }}

    .unit-metrics {{ display: flex; gap: 6px; margin-top: 2px; font-size: 10px; }}
    .metric-pill {{ background: rgba(255, 255, 255, 0.05); padding: 2px 6px; border-radius: 4px; color: var(--text-secondary); }}
    ...
    .legend-control {{
      ...
      font-size: 10px; width: 220px; box-shadow: var(--shadow);
    }}
    .legend-title {{ font-family: var(--font-heading); font-weight: 700; margin-bottom: 4px; font-size: 11px; color: var(--accent-primary); }}
    .legend-scale {{ height: 6px; border-radius: 3px; background: linear-gradient(to right, #2a9d8f, #e9c46a, #f4a261, #e63946, #d90429); margin-bottom: 4px; }}
    .legend-labels {{ display: flex; justify-content: space-between; font-size: 9px; color: var(--text-secondary); margin-bottom: 6px; }}
    ...
    .mobile-bar {{
      ...
      backdrop-filter: blur(16px); border-top: 1px solid var(--border-color);
      z-index: 1100; justify-content: space-around; align-items: center;
      padding: 0 8px;
    }}
    ...
    .modal-card {{
      ...
      border-radius: 16px; width: 800px; max-width: 100%; max-height: 90vh;
      overflow-y: auto; padding: 22px; box-shadow: var(--shadow);
    ...
    .close-btn {{ background: none; border: none; color: var(--text-secondary); font-size: 24px; cursor: pointer; padding: 4px 8px; }}

    .conclusion-grid {{ display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; }}
    .conclusion-card {{
    ...
    .conclusion-card-number {{ font-family: var(--font-heading); font-size: 24px; font-weight: 800; color: var(--accent-primary); }}
    .conclusion-card-title {{ font-family: var(--font-heading); font-size: 12px; font-weight: 700; color: var(--text-primary); }}
    .conclusion-card-desc {{ font-size: 11px; color: var(--text-secondary); line-height: 1.35; }}
    ...
    .block-header {{ font-family: var(--font-heading); font-size: 14px; font-weight: 700; color: var(--accent-warning); display: flex; align-items: center; gap: 6px; }}
    .bullet-list {{ display: flex; flex-direction: column; gap: 6px; font-size: 12px; color: var(--text-secondary); line-height: 1.45; padding-left: 16px; }}
    ...
    .info-box-title {{ font-family: var(--font-heading); font-size: 14px; font-weight: 700; display: flex; align-items: center; gap: 6px; }}
    .info-box-title.ndvi {{ color: var(--accent-green); }}
    .info-box-title.iecs {{ color: var(--accent-warning); }}

    .scale-table {{ width: 100%; border-collapse: collapse; margin-top: 6px; font-size: 11px; }}
    .scale-table th, .scale-table td {{ padding: 6px 8px; text-align: left; border-bottom: 1px solid rgba(255, 255, 255, 0.08); }}
    .scale-table th {{ background: rgba(255, 255, 255, 0.05); color: var(--text-primary); font-weight: 600; }}
    ...
    @media (max-width: 768px) {{
      ...
      .btn-header {{ padding: 5px 9px; font-size: 11px; }}

      /* Sidebar becomes a Sliding Drawer / Bottom Sheet on mobile */
      .app-sidebar {{
        position: fixed; top: 52px; left: 0; width: 100vw;
        height: calc(100vh - 108px); height: calc(100dvh - 108px);
        transform: translateY(100%);
        z-index: 1050; border-right: none;
      ...
      main {{
        flex: 1; height: calc(100vh - 108px); height: calc(100dvh - 108px);
      }}

      .mobile-bar {{ display: flex; }}

      /* Legend Control positioning on Mobile */
      .leaflet-bottom.leaflet-right {{
        bottom: 60px !important; right: 8px !important;
      }}
      .legend-control {{
        width: 170px; padding: 8px; font-size: 9px;
      }}
      .legend-title {{ font-size: 10px; margin-bottom: 3px; }}

      /* Modal Mobile Optimization */
      .modal-card {{
        padding: 16px; border-radius: 12px; max-height: 92vh;
      }}
      .modal-title {{ font-size: 16px; }}
      .conclusion-grid {{ grid-template-columns: 1fr; gap: 8px; }}
      .conclusion-card-number {{ font-size: 20px; }}
      .scale-table {{ font-size: 10px; }}
      .scale-table th, .scale-table td {{ padding: 4px 6px; }}
    }}
  </style>
</head>
<body>
  ...
  <div class="app-container">
    ...
    <header class="app-header">
      ...
      <div class="header-actions">
        <button class="btn-header highlight" id="btn-open-conclusions">💡 Conclusões</button>
        <button class="btn-header" id="btn-open-help">📖 Legendas</button>
        <div class="header-badges">
          <div class="badge">Rede SUS: <span>41 postos</span></div>
          <div class="badge">Buffer: <span>300m</span></div>
        </div>
      </div>
    </header>

    <!-- Sidebar Dashboard / Mobile Drawer -->
  ...
  <nav class="mobile-bar">
    <button class="mobile-nav-btn active" id="nav-map">
      <span>🗺️</span> Mapa
    </button>
    <button class="mobile-nav-btn" id="nav-list">
      <span>📋</span> Ranking SUS
    </button>
    <button class="mobile-nav-btn" id="nav-conclusions">
      <span>💡</span> Conclusões
    </button>
    <button class="mobile-nav-btn" id="nav-legend">
      <span>📖</span> Legendas
    </button>
  </nav>
  ...
  <div class="modal-overlay" id="conclusions-modal">
    <div class="modal-card">
      <div class="modal-header">
        <div class="modal-title">💡 Principais Conclusões da Análise Espacial</div>
      ...
      <div class="conclusion-grid">
        <div class="conclusion-card">
          ...
          <div class="conclusion-card-desc">19 das 41 unidades públicas de saúde (UBS, USF, UPAs) estão em faixas de Risco Alto ou Crítico de exposição térmica.</div>
        </div>
        <div class="conclusion-card">
          ...
          <div class="conclusion-card-desc">Registrado na periferia Noroeste (Selmi Dei / Valle Verde / Cecap), onde a ausência de arborização potencializa a ilha de calor.</div>
        </div>
        <div class="conclusion-card">
      ...
      <div class="section-block">
        ...
        <ul class="bullet-list">
          <li><strong>Disparidade Norte-Sul Evidente:</strong> A periferia Norte e Noroeste (Selmi Dei, Valle Verde, Cecap, Laranjeiras) concentra os maiores índices de risco térmico e social. Por outro lado, postos localizados no eixo Centro-Sul desfrutam de microclimas até 6°C mais amenos devido ao efeito protetor dos parques urbanos.</li>
          <li><strong>Sobreposição de Vulnerabilidades:</strong> As áreas com menor cobertura vegetal (NDVI &lt; 0.25) coincidem exatamente com os setores censitários do IBGE de maior vulnerabilidade socioeconômica e maior densidade demográfica.</li>
        </ul>
      </div>
      ...
      <div class="section-block">
        ...
        <ul class="bullet-list">
          ...
          <li><strong>Falta de Cobertura de Caminhada (300m):</strong> A maioria das USFs de bairros periféricos não possui praças ou vegetação de grande porte num raio de 300m de caminhada do usuário.</li>
        </ul>
      </div>

      <div class="section-block">
        <div class="block-header" style="color: var(--accent-danger);">🚀 3. Diretrizes de Ação para a Gestão Pública</div>
        <ul class="bullet-list">
          <li><strong>1. Arborização Urbana Prioritária:</strong> Plantio imediato de mudas de árvores de médio/grande porte nas calçadas num raio de 300m das unidades do <strong>Top 5 Crítico</strong> (USF Roxo, UPA Valle Verde, Posto Selmi Dei, PSF Neuza Dicenzo e PSF Adolfo Leão).</li>
          <li><strong>2. Soluções Baseadas na Natureza (SbN):</strong> Implantação de telhados verdes, pergolados arborizados nas áreas de espera externas e jardins de chuva no pátio dos postos.</li>
          <li><strong>3. Climatização & Salas de Hidratação para Ondas de Calor:</strong> Instalação prioritária de ar-condicionado e bebedouros de alta capacidade para atendimento de idosos e crianças durante ondas de calor extremo.</li>
        </ul>
      </div>
    </div>
  </div>
  ...
  <script>
        ...
        onEachFeature: (f, layer) => {{
          ...
          layer.bindPopup(`
            <div style="font-family: sans-serif; padding: 4px;">
              ...
              <p style="margin:2px 0; font-size:11px; color:#64748b;">Ação: Amortecimento térmico natural</p>
            </div>
          `);
        }}
      }}).addTo(map);
        ...
        onEachFeature: (f, layer) => {{
          ...
          layer.bindPopup(`
            <div style="font-family: sans-serif; min-width: 210px;">
              ...
              <h3 style="margin:0 0 6px 0; font-size:13px; color:#0f172a;">${{p.name}}</h3>
              <p style="margin:2px 0; font-size:11px;"><strong>Temp. Média (300m):</strong> ${{p.surface_temp_300m}}°C</p>
              <p style="margin:2px 0; font-size:11px;"><strong>Vegetação (NDVI 300m):</strong> ${{p.ndvi_300m}}</p>
              <p style="margin:2px 0; font-size:11px;"><strong>Vulnerabilidade Social:</strong> ${{p.vulnerability_score_300m}} / 5.0</p>
              <hr style="border:0; border-top:1px solid #e2e8f0; margin:6px 0;" />
              <div style="display:flex; justify-content:space-between; align-items:center;">
                <span style="font-size:11px;">Índice IECS:</span>
                <strong style="font-size:13px; color:${{riskColor}};">${{p.iecs_score}} / 100</strong>
              </div>
            </div>
          `);
        }}
    ...
    function setupMobileNavigation() {{
      ...
      const navConclusions = document.getElementById("nav-conclusions");
      const navLegend = document.getElementById("nav-legend");
      ...
      navConclusions.addEventListener("click", () => {{
        conclusionsModal.classList.add("active");
      }});

      navLegend.addEventListener("click", () => {{
    ...
    function addLegend() {{
      ...
      legend.onAdd = function() {{
        ...
        div.innerHTML = `
          ...
          <div style="font-size:9px; color:#94a3b8; display:flex; flex-direction:column; gap:2px;">
            <span>🟢 <strong>Áreas Verdes:</strong> Parques</span>
            <span>⚪ <strong>Buffer:</strong> 300m</span>
            <span>🔴 <strong>Pontos:</strong> Postos SUS</span>
          </div>
        `;
        return div;
      }};
...
"""
    ...
    print(f"Mobile-first standalone HTML generated at: {OUTPUT_HTML}")
